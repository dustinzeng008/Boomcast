...
@interface YZFollowViewController ()<UITableViewDelegate, UITableViewDataSource,
...
@implementation YZFollowViewController
...
- (void)loadFollowBroadcast{
    _broadcastFrames = [[NSMutableArray alloc] init];
    [MBProgressHUD showHUDAddedTo:self.view animated:YES];
    [[[self.databaseRef child:@"user_follow"] child:[User sharedInstance].uid] observeEventType:FIRDataEventTypeValue withBlock:^(FIRDataSnapshot * _Nonnull snapshot) {
        [MBProgressHUD hideHUDForView:self.view animated:YES];
        NSEnumerator *children = [snapshot children];
        FIRDataSnapshot *child;
        while (child = [children nextObject]) {
            FIRDatabaseQuery *query = [[[self.databaseRef child:@"user-broadcasts"] child:child.value] queryLimitedToLast:20];
